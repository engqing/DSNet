...
class RandomTranslatePoints(object):
    def __init__(self, parameters):
        self.range = parameters['range'] if 'range' in parameters else 0.1

    def __call__(self, ptcloud, rnd_value):
        translation = 2 * rnd_value - 1  # 将rnd_value映射到[-1, 1]范围
        translation *= self.range  # 缩放平移量
        ptcloud[:, :3] += translation
        return ptcloud
